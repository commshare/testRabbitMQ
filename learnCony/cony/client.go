...
type ClientOpt func(*Client)  /*这个类型是可选功能类型？？看起来是一个函数啊*/

// Client is a Main AMQP client wrapper
type Client struct {
	addr         string
	declarations []Declaration
	consumers    map[*Consumer]struct{}  /*支持多个消费者和生产者？？*/
	publishers   map[*Publisher]struct{}
	errs         chan error /*初始化为100个错误*/
	blocking     chan amqp.Blocking  /*用来通告tcp的流控的，表示服务器有内存或者磁盘告警*/
	run          int32        // bool
	conn         atomic.Value //*amqp.Connection
	bo           Backoffer /*是一个interface 用来做连接失败后的重连策略的*/
	attempt      int32 /*重试次数*/
	l            sync.Mutex  /*这个锁用来干啥的*/
	config       amqp.Config
}
...
func (c *Client) Declare(d []Declaration) {
	c.l.Lock()
	defer c.l.Unlock()
	/*把declaration存起来，这样每次这个客户端能获取到连接，就能re-run*/
...
func (c *Client) Close() {
	atomic.StoreInt32(&c.run, noRun/*0*/) // c.run = false  /*这个是0值啊，难道相当于变为false？*/
	conn, _ := c.conn.Load().(*amqp.Connection)
	if conn != nil {
		conn.Close() /*如果有连接存在，关闭连接*/
	}
	c.conn.Store((*amqp.Connection)(nil))  /*TODO  没搞懂啊，这里头弄个nil是干啥*/
}
...
func (c *Client) Loop() bool {
	...
	go func() {
		...
		for {
			select {
			case err1 := <-chanErr: /*出错了*/
				c.reportErr(err1) /*报告错误*/
				...
				/*从这个协程退出，以便可以启动重连接处理*/
				return
			case blocking := <-chanBlocking:  /*阻塞了*/
				select {
...
func (c *Client) reportErr(err error) bool {
	if err != nil { /*有错误*/
		select {
		case c.errs <- err:  /*不管能不能加入到errs这个chan里，都会返回true TODO */
		default:
		}
		return true
	}
...
func (c *Client) connection() (*amqp.Connection, error) {
	conn, _ := c.conn.Load().(*amqp.Connection)
	if conn == nil { /*没有store过，就会是nil的*/
		return nil, ErrNoConnection  /*用这个意思是有木有连接过*/
	}
